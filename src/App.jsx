...
// ── Validate one row against the column definitions. Returns { record, errors } ──
function validateRow(cells, columns) {
  ...
  columns.forEach((col, i) => {
    ...
    if (col.required && !raw) {
      errors.push(`"${col.label}" is required but empty`);
      record[col.key] = "";
      return;
    }
    if (!raw) { record[col.key] = ""; return; }

    if (col.type === "number") {
      if (!/^-?\d+(\.\d+)?$/.test(raw)) {
        errors.push(`"${col.label}" must be a plain number (got "${raw}")`);
      }
      record[col.key] = raw;
    } else if (col.type === "date") {
      const norm = parseFlexibleDate(raw);
      if (!norm) errors.push(`"${col.label}" must be a valid date, e.g. YYYY-MM-DD (got "${raw}")`);
      record[col.key] = norm || raw;
    } else if (col.type === "enum") {
      const match = col.enumValues.find((v) => v.toLowerCase() === raw.toLowerCase());
      if (!match) errors.push(`"${col.label}" must be one of: ${col.enumValues.join(", ")} (got "${raw}")`);
...
function ExcelImportButton({ moduleTitle, collectionName, columns, uniqueKeys = [], existingList = [], buildRecord, demoRows = [] }) {
  ...
  const [preview, setPreview] = useState(null); // { totalRows, validRows, invalidRows, duplicateRows }
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      for (const { rowNum, cells } of mapped) {
        const { record, errors } = validateRow(cells, columns);
        if (errors.length) {
          invalidRows.push({ rowNum, record, reason: errors.join("; ") });
          continue;
        }
        ...
        validRows.push({ rowNum, record });
      }

      setPreview({ totalRows: mapped.length, validRows, invalidRows, duplicateRows });
      setStage("preview");
    } catch (e) {
      setStage("idle");
          ...
          <div style={{ background: "white", borderRadius: 14, padding: "24px 26px", maxWidth: 560, width: "100%", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 8px 32px rgba(0,0,0,0.25)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 16 }}>
              {[
                ...
                { label: "Valid", val: preview.validRows.length, bg: "#e8f5e9", c: "#2e7d32" },
                { label: "Invalid", val: preview.invalidRows.length, bg: "#ffebee", c: "#c62828" },
            ...
            {preview.invalidRows.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 12, fontWeight: "bold", color: "#c62828", marginBottom: 4 }}>⚠️ Invalid rows (will be skipped)</div>
                <div style={{ maxHeight: 120, overflowY: "auto", background: "#fff5f5", borderRadius: 6, padding: "6px 10px" }}>
                  {preview.invalidRows.slice(0, 25).map((r, i) => (
                    <div key={i} style={{ fontSize: 11, color: "#b71c1c", padding: "2px 0" }}>Row {r.rowNum}: {r.reason}</div>
                  ))}
                  {preview.invalidRows.length > 25 && <div style={{ fontSize: 11, color: "#999" }}>+ {preview.invalidRows.length - 25} more...</div>}
                </div>
              </div>
            )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 16 }} onClick={closeAll}>
          <div style={{ background: "white", borderRadius: 14, padding: "28px 26px", maxWidth: 420, width: "100%", boxShadow: "0 8px 32px rgba(0,0,0,0.25)", textAlign: "center" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ textAlign: "left", fontSize: 13, color: "#444", background: "#f7f9fc", borderRadius: 8, padding: "12px 16px", lineHeight: 1.9 }}>
              ...
              <div style={{ color: "#c62828" }}>❌ Invalid rows skipped: <strong>{report.invalidRows}</strong></div>
