      ...
      <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 20 }}>
        ...
        <div style={{ background: "white", borderRadius: 12, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            ...
            <button onClick={() => showPreview(tableHTML("Receivings Report", ["Date","Source","Method","Category","Amount","Received By"], filtered.map(r=>[r.date,r.source,r.method,r.category,fmt(r.amount),r.receivedBy||"—"])))} style={{ ...btnSec, margin: 0 }}>🖨 Print</button>
          </div>
...
function CasesPage({ cases, crud, serials, workPlanEntries, expenses, loans }) {
  ...
  const calcCaseTotal = (caseNo) => {
    ...
    const total = expenses.filter(e => canonicalKey(e.serialNo) === key || canonicalKey(e.caseNo) === key).reduce((a,e)=>a+Number(e.amount||0),0);
...
<div class="outer">
  ...
  <table style="margin-top:0;border-top:none">
    <tr>
      ...
      <td colspan="3" style="font-weight:bold;color:#c00;font-size:13px">${c.totalAmount?Number(c.totalAmount).toLocaleString()+" Rs.":""}</td>
    </tr>
...
// ── Loans Page ────────────────────────────────────────────────────────────────
function LoansPage({ loans, crud, serials, receivings, expenses }) {
  ...
  const save = async () => {
    ...
    if (editing) { /* already saved above */ }
      ...
      const askReceivings = () => {
        ...
        confirm(
          `Also add Rs.${Number(form.amount).toLocaleString()} to RECEIVINGS as a loan issued record?`,
      ...
      if (crud?.expenses) {
        confirm(
          `Add Rs.${Number(form.amount).toLocaleString()} loan given to "${form.borrowerName}" as an EXPENSE entry (category: Miscellaneous)?`,
          () => {
            crud.expenses.save({
              date: form.date,
  ...
  const toggleInstallment = (loanId, instId, currentInst) => {
    ...
    const askDonations = () => {
      ...
      confirm(
        `Also record this Rs.${Number(currentInst.amount).toLocaleString()} repayment from "${loan.borrowerName}" in DONATIONS? (linked note: "From Loans — ${loan.borrowerName}")`,
        () => crud.donations.save({
          date: today(),
    ...
    if (crud?.receivings) {
      confirm(
        `Installment of Rs.${Number(currentInst.amount).toLocaleString()} received from "${loan.borrowerName}". Add to RECEIVINGS?`,
        () => { crud.receivings.save({
          date: today(),
            ...
            <button onClick={() => {
              ...
              showPreview(tableHTML("Loans — Borrowers List",
                ...
                filteredLoans.map((l,i) => {
                  const totalRec = l.installments.reduce((a,inst)=>a+Number(inst.paidAmount||(inst.paid?inst.amount:0)),0);
                  ...
                  return [i+1, l.date, l.serialNo||"—", l.borrowerName, l.cnic||"—", l.phone||"—", l.address||"—",
                    ...
                    l.installments.length ? "Rs. "+Number(l.installments[0].amount).toLocaleString() : "—",
              ...
              const rows = [["#","Date","Serial","Borrower","CNIC","Phone","Address","Loan Amount","Monthly","Received","Remaining","Status"],
                ...filteredLoans.map((l,i) => {
                  ...
                  return [i+1,l.date,l.serialNo||"",l.borrowerName,l.cnic||"",l.phone||"",l.address||"",l.amount,
                    l.installments[0]?.amount||"",totalRec,Number(l.amount)-totalRec,l.status];
                })];
      ...
      <table style="width:100%;border-collapse:collapse;font-size:10px">
        ...
        <tbody>
          ${v.entries.map((d,i)=>`<tr style="background:${i%2===0?"white":"#fff9fb"}">
            ...
            <td style="padding:4px 8px;border:1px solid #eee;text-align:right;font-weight:bold;color:#c2185b">Rs. ${Number(d.amount||0).toLocaleString()}</td>
...
function DonationsPage({ donations, crud }) {
  ...
  const printBriefSummary = () => {
    ...
    const grandTotal = filtered.reduce((a,d)=>a+Number(d.amount||0),0);
...
function loanMonthlyHTML(loan) {
  const MONTH_NAMES = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
  const totalPaid = loan.installments.reduce((a,i)=>a+Number(i.paidAmount||(i.paid?i.amount:0)),0);
  const remaining = Number(loan.amount) - totalPaid;
  const overdueCount = loan.installments.filter(i=>!i.paid && i.dueDate < new Date().toISOString().split("T")[0]).length;
  ...
  const rows = sortedMonths.map((ym,idx) => {
    ...
    return insts.map((i,j) => {
      ...
      return `<tr style="background:${i.paid?"#f9fff9":i.dueDate < new Date().toISOString().split("T")[0]?"#fff8f0":"white"}">
        ...
        <td>${i.dueDate}</td>
        <td><strong>Rs. ${Number(i.amount).toLocaleString()}</strong></td>
  ...
  const progress = loan.amount > 0 ? Math.round((totalPaid/Number(loan.amount))*100) : 0;
...
<div class="meta">
  <div class="meta-item">
    ...
    <div class="meta-value" style="color:#1565C0">Rs. ${Number(loan.amount).toLocaleString()}</div>
  </div>
  <div class="meta-item">
...
<div style="display:flex;justify-content:space-between;font-size:11px;color:#888;margin-bottom:4px">
  <span>Repayment Progress</span><span style="font-weight:bold;color:#1565C0">${progress}% (${loan.installments.filter(i=>i.paid).length}/${loan.installments.length} paid)</span>
</div>
...
<tfoot>
  <tr style="background:#f5f5f5;font-weight:bold">
    ...
    <td>Rs. ${Number(loan.amount).toLocaleString()}</td>
...
function SummaryPage({ receivings, expenses, cases, loans, serials, donations, crud }) {
  ...
  expAll.forEach(e => {
    ...
    bySerial[key].total += Number(e.amount||0);
  });
