...
function downloadExcelTemplate(moduleTitle, columns, demoRows) {
  ...
  const notesAoa = [
    ...
    [],
    ["General notes:"],
    ["• Do not rename or reorder the header row on the Data sheet — column order doesn't matter, but the header text must match."],
  ...
  wsNotes["!cols"] = [{ wch: 20 }, { wch: 10 }, { wch: 34 }, { wch: 60 }];
...
function mapRowsToColumns(rows, columns) {
  ...
  const first = rows[0].map((c) => xlCellToStr(c).toLowerCase());
