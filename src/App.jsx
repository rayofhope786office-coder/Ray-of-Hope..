        ...
        <div style={{ background: "white", borderRadius: 12, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap:"wrap" }}>
            ...
            <CSVImportButton
              ...
              uniqueField={{ rowKey: "code", existingList: serials, existingKey: "code", label: "Code" }}
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            ...
            <CSVImportButton
              ...
              columns={[
                ...
                { key: "source", label: "Source", required: true, type: "text" },
                { key: "method", label: "Method", required: true, type: "enum", enumValues: PAYMENT_METHODS },
                { key: "accountRef", label: "Ref No", required: false, type: "text" },
                { key: "category", label: "Category", required: true, type: "enum", enumValues: CATS },
                { key: "amount", label: "Amount", required: true, type: "number" },
                { key: "receivedBy", label: "Received By", required: false, type: "text" },
                { key: "description", label: "Description", required: false, type: "text" },
              ]}
        ...
        <div style={{ background: "white", borderRadius: 12, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            ...
            <CSVImportButton
              ...
              columns={[
                { key: "date", label: "Date", required: true, type: "text" },
                { key: "serialNo", label: "Serial", required: false, type: "text" },
                { key: "description", label: "Description", required: true, type: "text" },
                { key: "category", label: "Category", required: true, type: "enum", enumValues: EXP_CATS },
                { key: "amount", label: "Amount", required: true, type: "number" },
                { key: "paidTo", label: "Paid To", required: false, type: "text" },
                { key: "approvedBy", label: "Approved By", required: false, type: "text" },
              ]}
        ...
        <div style={{ background:"white", borderRadius:12, padding:20, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
          <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap" }}>
            ...
            <CSVImportButton
              ...
              columns={[
                { key: "caseNo", label: "Case No", required: true, type: "text" },
                { key: "titleOfCase", label: "Title", required: true, type: "text" },
                { key: "whatDone", label: "What Done", required: false, type: "text" },
                { key: "category", label: "Category", required: false, type: "text" },
                { key: "name", label: "Name", required: false, type: "text" },
                { key: "mobile", label: "Mobile", required: false, type: "text" },
                { key: "focalPersonName", label: "Focal Person", required: false, type: "text" },
                { key: "status", label: "Status", required: false, type: "text" },
                { key: "totalAmount", label: "Total Amount", required: false, type: "number" },
              ]}
              uniqueField={{ rowKey: "caseNo", existingList: cases, existingKey: "caseNo", label: "Case No" }}
              buildRecord={(record) => ({
                ...record,
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap:"wrap" }}>
            ...
            <CSVImportButton
              ...
              columns={[
                { key: "date", label: "Date", required: true, type: "text" },
                { key: "serialNo", label: "Serial", required: false, type: "text" },
                { key: "borrowerName", label: "Borrower", required: true, type: "text" },
                { key: "cnic", label: "CNIC", required: false, type: "text" },
                { key: "phone", label: "Phone", required: false, type: "text" },
                { key: "address", label: "Address", required: false, type: "text" },
                { key: "amount", label: "Loan Amount", required: true, type: "number" },
                { key: "months", label: "Months", required: false, type: "number" },
              ]}
              buildRecord={(record) => {
          ...
          <div style={{ background:"white", borderRadius:12, padding:20, boxShadow:"0 2px 8px rgba(0,0,0,0.08)", flex:1 }}>
            <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap" }}>
              ...
              <CSVImportButton
                ...
                columns={[
                  { key: "date", label: "Date", required: true, type: "text" },
                  { key: "donorName", label: "Donor", required: true, type: "text" },
                  { key: "donorPhone", label: "Phone", required: false, type: "text" },
                  { key: "purpose", label: "Purpose", required: false, type: "text" },
                  { key: "category", label: "Type", required: true, type: "enum", enumValues: DON_CATS },
                  { key: "method", label: "Method", required: true, type: "enum", enumValues: DON_METHODS },
                  { key: "amount", label: "Amount", required: true, type: "number" },
                  { key: "receivedBy", label: "Received By", required: false, type: "text" },
                ]}
                buildRecord={(record) => ({
        ...
        <div style={{ display:"flex", gap:8, marginBottom:14, flexWrap:"wrap", alignItems:"center" }}>
          ...
          <CSVImportButton
            ...
            columns={[
              ...
              { key: "problemIssue", label: "Problem", required: false },
              { key: "caseNo", label: "Case No", required: false },
              { key: "verification", label: "Verification", required: false },
              { key: "caseStatus", label: "Status", required: false },
            ]}
            validate={(record) => record.caseNo && entries.some(e => e.caseNo === record.caseNo) ? `Case No "${record.caseNo}" already exists in Work Plan` : null}
            onImportRow={async (record) => {
              await crud.workplan.save({
...
// ── CSV parsing helper — handles quoted fields, commas inside quotes, CRLF ──
...
// ── Reusable CSV Import button + modal ───────────────────────────────────────
// Rewritten for production use: validates every row up-front (no partial
// uploads on bad data), checks for duplicates against live Firestore data,
// and commits everything via writeBatch in chunks of 500 instead of one
// addDoc/setDoc call per row.
//
// Props:
//   columns:      [{ key, label, required, type: "text"|"number"|"enum",
//                     enumValues: [...] (case-sensitive, only for type:"enum") }]
//   collectionName: Firestore collection to write into (e.g. "receivings")
//   buildRecord:  (validatedRow) => finalRecordObject   — shape the row into
//                  whatever the rest of the app expects (defaults, extra
//                  fields, etc.) after validation has already passed.
//   uniqueField:  { rowKey, existingList, existingKey, label } — if provided,
//                  every row's rowKey is checked against existingList (the
//                  live array already loaded from Firestore in this page)
//                  before any writes happen, so duplicates are caught with
//                  zero extra reads and zero race conditions.
function CSVImportButton({ label = "📥 Import CSV", columns, collectionName, buildRecord, uniqueField }) {
  const confirm = useConfirm();
  const fileRef = useRef(null);
  const [isImporting, setIsImporting] = useState(false);
  const [status, setStatus] = useState("");       // progressive status text shown on the overlay
  const [result, setResult] = useState(null);      // final success/failure summary

  // ── Step 1: strict per-row validation — nothing is written until every
  // single row in the file passes. One bad row halts the whole operation
  // and reports the exact row number and reason, rather than importing
  // half a file and leaving the user unsure what actually landed.
  const validateRows = (dataRows, headerOffset) => {
    const seenInFile = new Set(); // catch duplicates *within* the file itself
    const validated = [];
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      for (let c = 0; c < columns.length; c++) {
        ...
        if (col.required && !raw) {
          return { error: `Row ${rowNum}: "${col.label}" is required but empty.` };
        }

        if (col.type === "number" && raw) {
          // Strict clean-number check — reject "12,000", "Rs 500", "N/A", etc.
          if (!/^-?\d+(\.\d+)?$/.test(raw)) {
            return { error: `Row ${rowNum}: "${col.label}" must be a plain number (got "${raw}").` };
          }
        }

        if (col.type === "enum" && raw) {
          // Case-sensitive strict match against the allowed list
          if (!col.enumValues.includes(raw)) {
            return { error: `Row ${rowNum}: "${col.label}" must be exactly one of [${col.enumValues.join(", ")}] (got "${raw}"). Check for typos or mismatched capitalization.` };
          }
        }
      ...
      if (uniqueField) {
        const val = record[uniqueField.rowKey];
        if (val) {
          if (seenInFile.has(val)) {
            return { error: `Row ${rowNum}: "${uniqueField.label}" value "${val}" appears more than once in this file.` };
          }
          seenInFile.add(val);
        }
      }

      validated.push({ rowNum, record });
    }
    return { validated };
  };

  // ── Step 2: active duplicate check against what's already live in Firestore.
  // Uses the collection data already subscribed on this page (via onSnapshot),
  // so this is instant and needs no extra network round-trip.
  const checkExistingDuplicates = (validated) => {
    if (!uniqueField) return null;
    for (const { rowNum, record } of validated) {
      const val = record[uniqueField.rowKey];
      if (!val) continue;
      const clash = uniqueField.existingList.some(item => item[uniqueField.existingKey] === val);
      if (clash) {
        return `Row ${rowNum}: "${uniqueField.label}" value "${val}" already exists in the database. Import stopped — no rows were written.`;
      }
    }
    return null;
  };
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      if (!rows.length) {
        setIsImporting(false); setStatus("");
        return confirm("The file appears to be empty.", null);
      }

      // Detect and skip a header row if it matches our expected column labels
      const firstRowLower = rows[0].map(c => c.trim().toLowerCase());
      const labelsLower = columns.map(c => c.label.toLowerCase());
      const looksLikeHeader = firstRowLower.some(c => labelsLower.includes(c));
      const dataRows = looksLikeHeader ? rows.slice(1) : rows;
      const headerOffset = looksLikeHeader ? 2 : 1;

      // ── Validate everything before touching the network ──
      setStatus("Validating data...");
      const { error: validationError, validated } = validateRows(dataRows, headerOffset);
      if (validationError) {
        setIsImporting(false); setStatus("");
        return confirm(`Import halted — no rows were written.\n\n${validationError}\n\nPlease fix this row in your spreadsheet and re-upload.`, null);
      }

      // ── Active duplicate check against live Firestore data ──
      setStatus("Checking for duplicates...");
      const dupError = checkExistingDuplicates(validated);
      if (dupError) {
        setIsImporting(false); setStatus("");
        return confirm(`Import halted — no rows were written.\n\n${dupError}`, null);
      }

      // ── Bulk commit via writeBatch, chunked to Firestore's 500-op limit ──
      const BATCH_SIZE = 500;
      const chunks = [];
      for (let i = 0; i < validated.length; i += BATCH_SIZE) chunks.push(validated.slice(i, i + BATCH_SIZE));

      let written = 0;
      for (let b = 0; b < chunks.length; b++) {
        setStatus(chunks.length > 1 ? `Uploading batch ${b + 1} of ${chunks.length}...` : `Uploading ${chunks[b].length} record${chunks[b].length !== 1 ? "s" : ""}...`);
        const batch = writeBatch(db);
        chunks[b].forEach(({ record }) => {
          const finalRecord = buildRecord(record);
          const ref = doc(collection(db, collectionName)); // auto-generated ID
          batch.set(ref, { ...finalRecord, createdAt: new Date().toISOString() });
        });
        await batch.commit();
        written += chunks[b].length;
      }

      setStatus("Import Successful!");
      setResult({ success: written, total: validated.length });
    } catch (e) {
      setResult({ success: 0, total: 0, fatalError: e.message || "Something went wrong reading this file." });
    }

    setTimeout(() => { setIsImporting(false); setStatus(""); }, 600); // let "Import Successful!" show briefly
  ...
  return (
    <>
      <button onClick={() => fileRef.current?.click()} disabled={isImporting}
        style={{ ...btnSec, margin:0, background:"#e8f5e9", color:"#2e7d32", border:"1px solid #a5d6a7", cursor: isImporting ? "wait" : "pointer", opacity: isImporting ? 0.7 : 1 }}>
        {isImporting ? "Importing..." : label}
      </button>
      <input ref={fileRef} type="file" accept=".csv,text/csv" style={{ display:"none" }}
        onChange={e => handleFile(e.target.files[0])} />

      {/* Full-screen blocking overlay while the import is running */}
      {isImporting && (
        <div style={{ position:"fixed", inset:0, background:"rgba(255,255,255,0.85)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", zIndex:10000 }}>
          ...
          <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
          <div style={{ fontFamily:"Georgia,serif", fontSize:15, color:"#1565C0", fontWeight:"bold" }}>{status}</div>
        </div>
      )}
      ...
      {result && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999, padding:16 }}
          ...
          <div style={{ background:"white", borderRadius:14, padding:"28px 26px", maxWidth:420, width:"100%", boxShadow:"0 8px 32px rgba(0,0,0,0.25)", textAlign:"center" }}
            onClick={e => e.stopPropagation()}>
            {result.fatalError ? (
              <>
                <div style={{ fontSize:36, marginBottom:10 }}>⚠️</div>
                <h3 style={{ margin:"0 0 8px", color:"#c62828", fontSize:16 }}>Import Failed</h3>
                <p style={{ fontSize:13, color:"#666" }}>{result.fatalError}</p>
              </>
            ) : (
              <>
                <div style={{ fontSize:36, marginBottom:10 }}>✅</div>
                <h3 style={{ margin:"0 0 8px", color:"#2e7d32", fontSize:16 }}>Import Successful</h3>
                <p style={{ fontSize:13, color:"#666" }}><strong>{result.success}</strong> of <strong>{result.total}</strong> rows imported.</p>
              </>
            )}
            <button onClick={() => setResult(null)} style={{ ...btnPrimary, marginTop:16, width:"100%" }}>Close</button>
          </div>
        </div>
      )}
    </>
