...
function SerialsPage({ serials, crud, cases, workPlanEntries }) {
  ...
  const save = async () => {
    ...
    if (editing) { await crud.serials.save(form);
        ...
        <div style={{ background:"white", borderRadius:12, padding:20, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
          <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap" }}>
            ...
            <button onClick={()=>{
              ...
              const allHTML = filtered.slice().reverse().map(c=>initialFormHTML(c)).join('<div style="page-break-after:always"></div>');
              showPreview(allHTML);
...
function LoansPage({ loans, crud, serials, receivings, expenses }) {
  ...
  const save = async () => {
    ...
    if (editing) { await crud.loans.save(record); }
      ...
      // Ask whether to add loan amount to Expenses
      if (crud?.expenses) {
        confirm(
      ...
      // Ask whether to add loan amount to Receivings
      if (crud?.receivings) {
        confirm(
      ...
      // Ask whether this loan disbursement should ALSO be logged as a Donation
      if (crud?.donations) {
        confirm(
          `Should Rs.${Number(form.amount).toLocaleString()} also be recorded in DONATIONS? (linked note: "From Loans — ${form.borrowerName}")`,
          () => crud.donations.save({
            date: form.date,
            donorName: form.borrowerName,
            ...
            inKindDetails: "",
            receivedBy: "",
            notes: `From Loans — ${form.borrowerName} (Serial: ${form.serialNo||"N/A"})`,
  ...
  const toggleInstallment = (loanId, instId, currentInst) => {
    ...
    // When marking paid, ask whether to add to Receivings
    if (nowPaid && crud?.receivings) {
    ...
    // When marking paid, also ask whether this repayment should count as a Donation
    if (nowPaid && crud?.donations) {
      const loan = loans.find(l => l.id === loanId);
      if (loan) {
        confirm(
          `Should this installment of Rs.${Number(currentInst.amount).toLocaleString()} from "${loan.borrowerName}" also be recorded in DONATIONS? (linked note: "From Loans — ${loan.borrowerName}")`,
          () => crud.donations.save({
            date: today(),
        ...
        <div style={{ background: "white", borderRadius: 12, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap:"wrap" }}>
            ...
            <button onClick={() => {
              ...
              const pages = loans.map(l => loanMonthlyHTML(l)).join('<div style="page-break-after:always"></div>');
              showPreview(pages);
...
function WorkPlanPage({ globalYear, setGlobalYear, cases, crud, serials, workPlanEntries }) {
  ...
  const setEntries = (fn) => {
    const next = typeof fn === 'function' ? fn(workPlanEntries) : fn;
    next.forEach(e => { if (!workPlanEntries.find(x=>x.id===e.id)) crud.workplan.save(e); else crud.workplan.save(e); });
    workPlanEntries.filter(e=>!next.find(x=>x.id===e.id)).forEach(e=>crud.workplan.remove(e.id));
  };
  ...
  const promoteToCase = (e, { silent } = {}) => {
    ...
    if (existing) {
      ...
      crud.cases.save({
        ...c,
        name: c.name || caseData.name,
        mobile: c.mobile || caseData.mobile,
        titleOfCase: c.titleOfCase || caseData.titleOfCase,
        whatDone: c.whatDone || caseData.whatDone,
        focalPersonName: c.focalPersonName || caseData.focalPersonName,
        cnicCopy: c.cnicCopy || caseData.cnicCopy,
        purchasing: c.purchasing || caseData.purchasing,
        videosPics: c.videosPics || caseData.videosPics,
        posted: c.posted || caseData.posted,
        // Update facts from description if case facts are all blank
        facts: c.facts?.some(f=>f) ? c.facts : caseData.facts,
        workPlanLinked: true,
      });
  ...
  const save = async () => {
    ...
    if (editing) setEntries(p=>p.map(e=>e.id===editing?{...form}:e));
    else setEntries(p=>[...p,{...form,id:uid()}]);

    if (justAccepted) promoteToCase(form);

    setForm({...BLANK,year:globalYear}); setEditing(null);
  };
  const del  = (id) => confirm("Delete this entry?",()=>setEntries(p=>p.filter(e=>e.id!==id)));
  ...
  // ── Inline status (Pending / In Progress / Closed) change — kept separate from Verification ──
  const setStatusInline = (entryId, newVal) => {
    setEntries(prev => prev.map(e => e.id===entryId ? {...e, caseStatus:newVal} : e));
  };
  ...
  const TC = ({entry,field}) => (
    <td style={{ textAlign:"center", padding:"4px 3px", borderBottom:"1px solid #eee" }}>
      <button onClick={()=>setEntries(p=>p.map(e=>e.id===entry.id?{...e,[field]:!e[field]}:e))}
...
function UsersPage({ users, crud, user }) {
  ...
  const del = (id) => { if (id === user.id) return confirm("You cannot delete your own account.", null); confirm("Delete this user?", () => crud.users.remove(id)); };
