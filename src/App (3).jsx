...
// ── Read one row into a record. Empty or oddly-formatted cells are never
// rejected — they're just imported as-is (blank, or best-effort raw text) —
// but we still collect soft "warnings" so the preview can flag them for
// the user's awareness without blocking the import. ──
function validateRow(cells, columns) {
  ...
  columns.forEach((col, i) => {
    ...
    if (col.type === "number") {
      if (!/^-?\d+(\.\d+)?$/.test(raw)) warnings.push(`"${col.label}" isn't a plain number (got "${raw}") — imported as-is`);
      record[col.key] = raw;
    } else if (col.type === "date") {
      const norm = parseFlexibleDate(raw);
      if (!norm) warnings.push(`"${col.label}" isn't a recognized date (got "${raw}") — imported as-is`);
      record[col.key] = norm || raw;
    } else if (col.type === "enum") {
      const match = col.enumValues.find((v) => v.toLowerCase() === raw.toLowerCase());
      if (!match) warnings.push(`"${col.label}" doesn't match an allowed value (got "${raw}") — imported as-is`);
...
function ExcelImportButton({ moduleTitle, collectionName, columns, uniqueKeys = [], existingList = [], buildRecord, demoRows = [] }) {
  ...
  const doImport = async () => {
    ...
    const chunks = [];
    for (let i = 0; i < preview.validRows.length; i += BATCH_SIZE) chunks.push(preview.validRows.slice(i, i + BATCH_SIZE));
    ...
    for (const chunk of chunks) {
      try {
        ...
        chunk.forEach(({ record }) => {
          ...
          batch.set(ref, { ...finalRecord, createdAt: new Date().toISOString() });
        });
        await batch.commit();
