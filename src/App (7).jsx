    ...
    <ConfirmCtx.Provider value={ask}>
      ...
      {state && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999, padding:16 }}>
          <div style={{ background:"white", borderRadius:14, padding:"28px 32px", minWidth:300, maxWidth:380, boxShadow:"0 8px 32px rgba(0,0,0,0.25)", textAlign:"center" }}>
            <div style={{ fontSize:36, marginBottom:12 }}>{state.onOk ? (state.destructive ? "⚠️" : "❓") : "ℹ️"}</div>
            <div style={{ fontSize:15, fontFamily:"Georgia,serif", color:"#333", marginBottom:22, lineHeight:1.5 }}>{state.msg}</div>
...
function SerialsPage({ serials, crud, cases, workPlanEntries }) {
  ...
  const save = async () => {
    ...
    const dupSerial = serials.find((s) => s.code === code && s.id !== form.id);
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 20 }}>
        <div style={{ background: "white", borderRadius: 12, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          ...
          {form.serialNo && (() => {
            const match = serials.find(s => s.code === form.serialNo);
...
// ── Cases Page ────────────────────────────────────────────────────────────────
function CasesPage({ cases, crud, serials, workPlanEntries, expenses }) {
  const BLANK = {
  ...
  const calcCaseTotal = (caseNo) => {
    ...
    const total = expenses.filter(e => e.serialNo === caseNo || e.caseNo === caseNo).reduce((a,e)=>a+Number(e.amount||0),0);
  ...
  const syncExpenseTotals = () => {
    cases.forEach(c => { const _u = {};
      ...
      const wpEntry   = workPlanEntries?.find(e => e.caseNo === c.caseNo);
  ...
  const resyncAllSerials = async () => {
    ...
    for (const c of cases) {
      ...
      const existing = serials.find(s => s.code === c.caseNo);
  ...
  const checkPurchasing = (caseNo) => {
    ...
    return expenses.some(e => e.serialNo === caseNo || e.caseNo === caseNo);
  };
  ...
  useEffect(() => {
    ...
    const wpEntry = workPlanEntries?.find(e => e.caseNo === form.caseNo);
    setForm(f => ({
      ...f,
  ...
  const handleCaseNoChange = (val) => {
    setForm(f => {
      ...
      const wpEntry = workPlanEntries?.find(e => e.caseNo === val);
      return {
        ...f,
        caseNo: val,
  ...
  const save = async () => {
    ...
    const dup = cases.find(c => c.caseNo === caseNo && c.id !== editing);
    ...
    if (!editing) {
      const wpOwner = workPlanEntries?.find(e => e.caseNo === caseNo);
    ...
    try {
      ...
      if (caseNo && crud?.serials) {
        ...
        let existing = serials.find(s => s.code === caseNo);
  ...
  const edit = (c) => {
    ...
    const wpEntry = workPlanEntries?.find(e => e.caseNo === loaded.caseNo);
      ...
      <div style={{ display:"grid", gridTemplateColumns:"360px 1fr", gap:20 }}>
        ...
        <div style={{ background:"white", borderRadius:12, padding:20, boxShadow:"0 2px 8px rgba(0,0,0,0.08)", maxHeight:"90vh", overflowY:"auto" }}>
          ...
          {form.caseNo && (() => {
            ...
            const matchCase = cases.find(c => c.caseNo === form.caseNo && c.id !== editing);
...
function LoansPage({ loans, crud, serials, receivings, expenses }) {
  ...
  const filteredLoans = loans.filter(l => {
    ...
    // Same natural sort as Serial Numbers / Cases / Work Plan
...
function DonationsPage({ donations, crud }) {
  ...
  const printBriefSummary = () => {
    ...
    filtered.forEach(d => {
      ...
      if (!byPurpose[p]) byPurpose[p] = { total:0, count:0 };
      byPurpose[p].total += Number(d.amount||0);
      byPurpose[p].count++;
    });
    const rows = Object.entries(byPurpose).sort((a,b)=>b[1].total-a[1].total)
      .map(([p,v])=>[p, v.count, "Rs. "+v.total.toLocaleString()]);
  ...
  const printDetailedSummary = () => {
    ...
    filtered.forEach(d => {
      const p = d.purpose || "General";
      if (!byPurpose[p]) byPurpose[p] = { total:0, count:0, entries:[] };
      byPurpose[p].total += Number(d.amount||0);
      byPurpose[p].count++;
      byPurpose[p].entries.push(d);
    });
...
function WorkPlanPage({ globalYear, setGlobalYear, cases, crud, serials, workPlanEntries }) {
  ...
  const save = async () => {
    ...
    if (!editing && caseNo) {
      const dupEntry = entries.find(x => x.caseNo === caseNo);
      ...
      const caseOwner = cases.find(c => c.caseNo === caseNo);
  ...
  const handleWPCaseNoChange = (val) => {
    setForm(f => {
      const matchingSerial = serials.find(s => s.code === val);
      const matchingCase = cases.find(c => c.caseNo === val);
      return {
        ...f,
        caseNo: val,
  ...
  const syncAllAccepted = () => {
    ...
    if (crud?.serials) {
      accepted.forEach(e => {
        ...
        const existing = serials.find(s => s.code === e.caseNo);
        crud.serials.save({
          id: existing?.id || safeId,
          code: e.caseNo,
...
function SummaryPage({ receivings, expenses, cases, loans, serials, donations, crud }) {
  ...
  expAll.forEach(e => {
    ...
    if (!bySerial[code]) bySerial[code] = { code, entries:[], total:0 };
    bySerial[code].entries.push(e);
    bySerial[code].total += Number(e.amount||0);
  });
  const serialRows = Object.values(bySerial).map(s => {
    // Description here always comes from the linked Serial Number's title
    // (auto-created from Cases, or entered directly in Serial Numbers).
    // If a serial code has no match yet, fall back to the code itself
    // rather than leaving the column blank.
    const serialTitle = s.code === "General" ? "General Expenses" : serials.find(x => x.code === s.code)?.title;
    return { ...s, title: serialTitle || s.code };
