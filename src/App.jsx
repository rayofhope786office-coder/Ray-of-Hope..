...
import { getFirestore, collection, onSnapshot, addDoc, updateDoc, deleteDoc, doc, setDoc, writeBatch, getDocs, query, where } from "firebase/firestore";
...
function SerialsPage({ serials, crud, cases, workPlanEntries }) {
  ...
  const save = async () => {
    ...
    if (!editing) {
      // Only check for duplicates when creating a NEW serial — editing an
      // existing one should never be blocked by its own code.
      ...
      if (otherOwners.length) {
        confirm(`"${form.code}" is already used in ${otherOwners.map(o=>o.source).join(" and ")} (${otherOwners.map(o=>o.title||"untitled").join(", ")}). Each Case/Serial No should be used once.`, null);
        return;
      }
    }
        ...
        <div style={{ background: "white", borderRadius: 12, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap:"wrap" }}>
            ...
            <ExcelImportButton
              ...
              buildRecord={(record) => ({ ...record, category: record.category || "General" })}
              demoRows={[
                { code: "26/1", title: "Sample Family Support Case", category: "Education", description: "School fee assistance" },
                { code: "Um01", title: "Umbrella Project A", category: "General", description: "" },
              ]}
            />
          </div>
...
function CasesPage({ cases, crud, serials, workPlanEntries, expenses }) {
  ...
  const handleCaseNoChange = (val) => {
    setForm(f => {
      ...
      // Also pull videosPics from matching Work Plan entry
  ...
  const save = async () => {
    ...
    // Block genuine duplicates (same Case No already in Cases)
    if (!editing) {
    ...
    if (!editing) {
      ...
      if (wpOwner && wpOwner.verification !== "Accepted") {
        confirm(`Note: "${form.caseNo}" exists in Work Plan but hasn't been marked Accepted yet (current: ${wpOwner.verification}). Saving here is fine — consider accepting it in Work Plan too so both stay in sync.`, null);
      }
    }
    ...
    if (form.caseNo && crud?.serials) {
      const safeId = "case_" + form.caseNo.replace(/[^a-zA-Z0-9_-]/g, "_");
      const existing = serials.find(s => s.code === form.caseNo);
      const serialData = {
        id: existing?.id || safeId,
        code: form.caseNo,
        title: form.titleOfCase || (existing?.title) || form.name || form.caseNo,
        description: existing?.description || ("Auto from Case: " + (form.name||"")),
        category: form.category || existing?.category || "General",
      };
...
function LoansPage({ loans, crud, serials, receivings, expenses }) {
  ...
  const save = async () => {
    ...
    if (editing) { await crud.loans.save(record); }
    else {
  ...
  const updateInstallment = (loanId, instId, changes) => {
    const updated = loans.map((l) => l.id === loanId ? {
      ...l, installments: l.installments.map((i) => i.id === instId ? { ...i, ...changes } : i)
    } : l);
    const changedLoan = updated.find(l => l.id === loanId);
    if (changedLoan) crud.loans.save(changedLoan);
...
function SummaryPage({ receivings, expenses, cases, loans, serials }) {
...
function UsersPage({ users, crud, user }) {
  ...
  const del = (id) => { if (id === user.id) return confirm("You cannot delete your own account.", null); confirm("Delete this user?", () => crud.users.remove(id), true); };
            ...
            <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
              ...
              <tbody>{users.map((u) => (
                <tr key={u.id} style={{ borderBottom: "1px solid #eee" }}>
                  <td style={td}>{u.username} {u.id === user.id && <span style={{ ...badge, background: "#e3f2fd", color: "#1565C0" }}>You</span>}</td>
                  <td style={td}>
            ...
            {users.map(u => (
              <div key={u.id} style={{ border:"1px solid #eee", borderRadius:10, padding:"12px 14px", marginBottom:10 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                  <div style={{ fontWeight:"bold", fontSize:14 }}>{u.username} {u.id === user.id && <span style={{ ...badge, background: "#e3f2fd", color: "#1565C0", marginLeft:4 }}>You</span>}</div>
