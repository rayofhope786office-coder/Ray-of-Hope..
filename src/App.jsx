...
export default function App() {
  ...
  const props = { serials, receivings, expenses, cases, loans, donations, users, crud, user: { ...userProfile, email: authUser.email, uid: authUser.uid } };
...
function ReceivingsPage({ receivings, crud }) {
  ...
  const save = async () => {
    ...
    if (editing) setReceivings((p) => p.map((r) => r.id === editing ? { ...form } : r));
    else setReceivings((p) => [...p, { ...form, id: uid() }]);
    setForm(BLANK); setEditing(null);
  };
  const del = (id) => confirm("Delete this receiving entry?", () => setReceivings((p) => p.filter((r) => r.id !== id)));
...
function CasesPage({ cases, crud, serials, workPlanEntries, expenses }) {
  ...
  const handleImg = async (field, file) => {
    ...
    setForm(f => {
      ...
      // ── Auto-check CNIC Copy when either CNIC photo is added ──────
      if (field === "cnicFront" || field === "cnicBack") {
        updated.cnicCopy = true;
      }
      // ── Auto-check Videos/Pics when any photo is added ────────────
      if (field === "photo" || field === "cnicFront" || field === "cnicBack") {
        updated.videosPics = true;
        // Sync videosPics back to Work Plan entry with same Case No
        // WorkPlan sync via Firestore handled by onSnapshot subscription
      }
      return updated;
    });
  };
  ...
  const save = async () => {
    ...
    // Runs every time a case is saved (new or edited) so Serials always
    // reflect what's in Cases without manual entry.
    if (form.caseNo) {
      if (crud?.serials) {
        const existing = serials.find(s => s.code === form.caseNo);
        if (existing) {
          crud.serials.save({ ...existing, title: form.titleOfCase || existing.title, description: existing.description || ("Auto from Case: " + (form.name||"")), });
        } else {
          crud.serials.save({ code: form.caseNo, title: form.titleOfCase || form.name || form.caseNo, description: "Auto from Case: " + (form.name||""), category: form.category || "General" });
        }
      }
    }
...
// ── Loans Page ────────────────────────────────────────────────────────────────
function LoansPage({ loans, crud, serials, receivings, setReceivings, expenses, setExpenses }) {
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap:"wrap" }}>
            ...
            <button onClick={() => {
              ...
              showPreview(tableHTML("Loans — Borrowers List",
                ["#","Date","Serial","Borrower Name","CNIC","Phone","Loan Amount","Monthly","Received","Remaining","Status"],
            ...
            <button onClick={() => {
              const rows = [["#","Date","Serial","Borrower","CNIC","Phone","Loan Amount","Monthly","Received","Remaining","Status"],
                ...loans.slice().reverse().map((l,i) => {
                  const totalRec = l.installments.reduce((a,inst)=>a+Number(inst.paidAmount||(inst.paid?inst.amount:0)),0);
                  return [i+1,l.date,l.serialNo||"",l.borrowerName,l.cnic||"",l.phone||"",l.amount,
...
function DonationsPage({ donations, setDonations }) {
  ...
  const syncAllAccepted = () => {
    ...
    if (crud?.serials) {
      accepted.forEach(e => {
        ...
        if (existing) {
          crud.serials.save({ ...existing, title: e.title || existing.title });
        } else {
          crud.serials.save({ code: e.caseNo, title: e.title || e.description || e.name || e.caseNo, description: "Work Plan entry", category: "General" });
        }
      });
    }
