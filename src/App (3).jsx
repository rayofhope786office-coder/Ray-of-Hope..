...
function LoginPage() {
  ...
  const handle = async () => {
    ...
    try {
      // "Remember me" checked -> stay logged in across browser restarts (local persistence).
      // Unchecked -> only stay logged in for this browser tab/session (cleared on close).
      ...
      <div style={{ background:"white", borderRadius:16, padding:"32px 28px", width:"100%", maxWidth:380, boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        {mode === "login" && (
          <>
            ...
            <div style={{ position:"relative" }}>
              ...
              />
              <button type="button" onClick={()=>setShowPass(s=>!s)} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", fontSize:18, color:"#888", padding:2 }}>
                {showPass ? "🙈" : "👁"}
              </button>
            </div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:12 }}>
              <label style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:"#555", cursor:"pointer" }}>
                <input type="checkbox" checked={rememberMe} onChange={e=>setRememberMe(e.target.checked)} style={{ width:15, height:15, accentColor:"#1565C0", cursor:"pointer" }} />
                Remember me
              </label>
              <button type="button" onClick={()=>{ setMode("reset"); setErr(""); setInfo(""); }} style={{ background:"none", border:"none", color:"#1565C0", fontSize:13, cursor:"pointer", fontFamily:"Georgia,serif", textDecoration:"underline", padding:0 }}>
                Forgot password?
              </button>
            </div>
        ...
        {mode === "reset" && (
          <>
            ...
            <button type="button" onClick={()=>{ setMode("login"); setErr(""); setInfo(""); }} style={{ width:"100%", marginTop:10, background:"none", border:"none", color:"#1565C0", fontSize:13, cursor:"pointer", fontFamily:"Georgia,serif", textDecoration:"underline", padding:6 }}>
              ← Back to login
            </button>
          </>
        )}
...
function CasesPage({ cases, crud, serials, workPlanEntries }) {
  // Note: Firebase version auto-syncs via Firestore, setSerials uses crud.serials.save
  const BLANK = {
  ...
  const save = async () => {
    ...
    // ── Auto-sync Case No + Title into Serial Numbers (Firestore) ──────
    if (form.caseNo && crud.serials) {
      const existing = serials.find(s => s.code === form.caseNo);
      if (existing) {
        await crud.serials.save({ ...existing, title: form.titleOfCase || existing.title, description: existing.description || ("Case: " + (form.name||"")) });
      } else {
        await crud.serials.save({ code: form.caseNo, title: form.titleOfCase || form.name || form.caseNo, description: "Case: " + (form.name||""), category: form.category || "General" });
      }
    }
